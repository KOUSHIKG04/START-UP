...
- Figma mobile flows use phone OTP. Do not replace them with email/password just because email is easier to configure. A guarded email/password login exists only for development builds pointed at the named disposable project; its server-created fixtures simulate confirmed phone ownership and do not validate SMS. The hospital/clinic web portal uses email/password; the SMS provider remains open.
...
Next: use the disposable-project fixture procedure in [plans/mobile-backend-integration.md](plans/mobile-backend-integration.md) to test mobile onboarding and permitted clinic/ambulance actions without SMS. Review and promote the full post-baseline migration set separately to the intended development project. Configure SMS, provision an authorized verified-email portal member, and later run real-device OTP/onboarding, clinic, ambulance and SOS journeys with separate patient/doctor/driver accounts. Keep SOS dispatch disabled until device and operational review pass. Keep applied migrations immutable. The historical clinic slice is documented in [plans/backend-clinic-first.md](plans/backend-clinic-first.md).
